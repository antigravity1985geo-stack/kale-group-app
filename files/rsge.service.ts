...
// ─── Supabase client (reuse your existing lib/supabase.ts) ──
// TODO: Import from your existing supabase client
...
export async function createWaybillForOrder(
  ...
  const payload: WaybillCreatePayload = {
    ...
    items: order.order_items.map((item: any, idx: number) => ({
      ...
      totalPrice:   parseFloat(item.price_at_purchase * item.quantity).toFixed(2) as unknown as number,
      vatRate:      18,
