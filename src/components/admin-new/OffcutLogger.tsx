...
function GlassInput({ label, unit, value, onChange, type = 'number', error, ...props }: any) {
  return (
              ...
              <motion.div key="log" initial={{ opacity: 0, x: 16 }} animate={{ opacity: 1, x: 0 }}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  ...
                  <button
                    ...
                    onMouseEnter={e => { (e.target as any).style.borderColor = G.gold; (e.target as any).style.color = G.gold }}
                    onMouseLeave={e => { (e.target as any).style.borderColor = G.glassBorder; (e.target as any).style.color = G.textMuted }}
                  >
                    + კიდევ ერთი ნარჩენი
                  </button>
