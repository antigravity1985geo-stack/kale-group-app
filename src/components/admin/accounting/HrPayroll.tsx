    ...
    <div className="space-y-6">
      ...
      {tab === 'payroll' && (
        <>
          ...
          {loading ? (
            <div className="space-y-2">{Array(3).fill(0).map((_, i) => <div key={i} className="h-16 bg-white shadow-sm animate-pulse rounded-xl" />)}</div>
          ) : runs.length === 0 ? (
            <div className="text-center py-16 text-slate-400"><DollarSign size={40} className="mx-auto mb-3 opacity-30" /><p>პეიროლ-ი ჯ. არ ჩატ.</p></div>
          ) : (
