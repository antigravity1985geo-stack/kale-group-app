  ...
  if (!isAdmin) {
    return (
      <div className="flex flex-col items-center justify-center p-12 text-center border border-red-500/20 bg-red-500/5 rounded-2xl">
        ...
        <p className="text-admin-muted max-w-md">
  ...
  return (
    <div className="admin-fade-in space-y-6">
      ...
      <div className="flex justify-between items-center bg-white shadow-sm/50 p-4 rounded-xl border border-admin-muted/10">
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2 text-admin-muted">
            ...
            <select
              ...
              className="bg-white shadow-sm border border-admin-muted/10 rounded-lg px-3 py-1.5 text-sm text-admin-text focus:border-gold-500 focus:bg-white transition-all shadow-sm transition-all outline-none"
            >
        ...
        <button onClick={fetchLogs} className="px-3 py-2 bg-admin-bg hover:bg-stone-700 text-admin-text rounded-lg transition-colors border-none cursor-pointer outline-none flex items-center justify-center">
      ...
      <div className="bg-white shadow-sm rounded-xl border border-admin-muted/10 overflow-hidden min-h-[400px]">
        {loading ? (
          <div className="flex flex-col items-center justify-center h-64 text-amber-500">
            ...
            <p className="text-sm font-bold tracking-widest uppercase text-admin-muted">ჟურნალი იტვირთება...</p>
          </div>
        ) : (
              ...
              <tbody className="divide-y divide-slate-200">
                {logs.map(log => (
                  <tr key={log.id} className="hover:bg-slate-100/50/50 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      {log.changed_by_user ? (
                        <div className="flex flex-col">
                          <span className="text-admin-text font-medium">{log.changed_by_user.full_name}</span>
                    ...
                    <td className="px-6 py-4 font-mono text-admin-muted text-xs">{log.table_name}</td>
                    <td className="px-6 py-4 font-mono text-admin-muted text-xs text-ellipsis overflow-hidden max-w-[100px]" title={log.record_id}>
                      {log.record_id?.slice(0,8)}...
                    </td>
                    <td className="px-6 py-4 text-right">
                       <button 
                         className="p-2 text-admin-muted hover:text-admin-text bg-admin-bg hover:bg-stone-700 rounded-lg transition-colors border-none cursor-pointer outline-none inline-flex items-center gap-2 text-xs"
