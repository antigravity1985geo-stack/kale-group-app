...
export default function VatModule() {
  ...
  return (
    <div className="admin-fade-in space-y-6">
      <div>
        <h2 className="text-2xl font-bold text-admin-text flex items-center gap-2"><Receipt size={22} /> დღგ მოდული</h2>
        <p className="text-admin-muted text-sm mt-1">Output / Input VAT · ყოველთვიური დეკლარაცია</p>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="bg-red-900/20 border border-red-800/40 rounded-xl p-5">
          <div className="flex items-center gap-2 mb-2">
            ...
            <p className="text-admin-muted text-xs">Output VAT (YTD)</p>
          </div>
        ...
        <div className="bg-emerald-900/20 border border-emerald-800/40 rounded-xl p-5">
          <div className="flex items-center gap-2 mb-2">
            ...
            <p className="text-admin-muted text-xs">Input VAT (YTD)</p>
          </div>
        ...
        <div className={`border rounded-xl p-5 ${ytdNet > 0 ? 'bg-amber-900/20 border-amber-800/40' : 'bg-emerald-900/10 border-emerald-800/30'}`}>
          <div className="flex items-center gap-2 mb-2">
            ...
            <p className="text-admin-muted text-xs">სანეტო (YTD)</p>
          </div>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {summary.filter(r => r.output_vat > 0 || r.input_vat > 0).map((r, i) => {
                  ...
                  return (
                    <tr key={i} className="border-b border-admin-muted/10/50 hover:bg-slate-100/50/20">
                      <td className="py-3 px-4 text-admin-text">{MONTH_NAMES[(r.period_month || 1) - 1]} {r.period_year}</td>
                      ...
                      <td className="py-3 px-4 text-center">
                        <span className="text-xs px-2 py-0.5 rounded-full bg-admin-bg text-admin-muted border border-admin-muted/10">DRAFT</span>
                      </td>
                    </tr>
                  );
                })}
              ...
              <tfoot>
                <tr className="border-t-2 border-admin-muted/10 font-semibold">
      ...
      {tab === 'transactions' && (
        <>
          <div className="flex gap-2">
            {['', 'OUTPUT', 'INPUT'].map(t => (
              <button key={t} onClick={() => setVatType(t)} className={`px-3 py-2 rounded-xl text-xs border transition-all ${vatType === t ? 'bg-admin-primary text-white border-admin-primary shadow-lg shadow-admin-primary/20' : 'border-admin-muted/10 text-admin-muted'}`}>
                {t || 'ყველა'}
              </button>
            ))}
          </div>
          {loading ? (
            ...
            <div className="space-y-2">
              {transactions.map((t, i) => (
                <div key={i} className="flex items-center gap-4 bg-white shadow-[0_8px_30px_rgba(0,0,0,0.02)] rounded-2xl border border-admin-muted/5 hover:shadow-lg transition-all px-5 py-3">
                  ...
                  <div className="text-right shrink-0">
                    <p className="text-admin-muted text-xs">{GEL(t.taxable_amount)} × {t.vat_rate}%</p>
                    <p className="font-semibold text-admin-text">{GEL(t.vat_amount)}</p>
                  </div>
