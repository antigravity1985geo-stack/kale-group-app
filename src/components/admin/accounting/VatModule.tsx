...
export default function VatModule() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold text-white flex items-center gap-2"><Receipt size={22} /> დღგ მოდული</h2>
        <p className="text-stone-400 text-sm mt-1">Output / Input VAT · ყოველთვიური დეკლარაცია</p>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="bg-red-900/20 border border-red-800/40 rounded-xl p-5">
          <div className="flex items-center gap-2 mb-2">
            ...
            <p className="text-stone-400 text-xs">Output VAT (YTD)</p>
          </div>
        ...
        <div className="bg-emerald-900/20 border border-emerald-800/40 rounded-xl p-5">
          <div className="flex items-center gap-2 mb-2">
            ...
            <p className="text-stone-400 text-xs">Input VAT (YTD)</p>
          </div>
        ...
        <div className={`border rounded-xl p-5 ${ytdNet > 0 ? 'bg-amber-900/20 border-amber-800/40' : 'bg-emerald-900/10 border-emerald-800/30'}`}>
          <div className="flex items-center gap-2 mb-2">
            ...
            <p className="text-stone-400 text-xs">სანეტო (YTD)</p>
          </div>
      ...
      {tab === 'summary' && (
        loading ? (
          <div className="space-y-2">{Array(4).fill(0).map((_, i) => <div key={i} className="h-14 bg-stone-900 animate-pulse rounded-xl" />)}</div>
        ) : summary.length === 0 ? (
          <div className="text-center py-16 text-stone-500"><Receipt size={40} className="mx-auto mb-3 opacity-30" /><p>VAT მონ. ჯერ არ არის</p></div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
                ...
                {summary.filter(r => r.output_vat > 0 || r.input_vat > 0).map((r, i) => {
                  ...
                  return (
                    <tr key={i} className="border-b border-stone-800/50 hover:bg-stone-800/20">
                      <td className="py-3 px-4 text-white">{MONTH_NAMES[(r.period_month || 1) - 1]} {r.period_year}</td>
                      ...
                      <td className="py-3 px-4 text-center">
                        <span className="text-xs px-2 py-0.5 rounded-full bg-stone-800 text-stone-400 border border-stone-700">DRAFT</span>
                      </td>
                    </tr>
                  );
                })}
              ...
              <tfoot>
                <tr className="border-t-2 border-stone-700 font-semibold">
                  <td className="py-3 px-4 text-stone-300 text-xs uppercase tracking-wider">YTD სულ</td>
      ...
      {tab === 'transactions' && (
        <>
          <div className="flex gap-2">
            {['', 'OUTPUT', 'INPUT'].map(t => (
              <button key={t} onClick={() => setVatType(t)} className={`px-3 py-2 rounded-xl text-xs border transition-all ${vatType === t ? 'bg-amber-600 border-amber-500 text-white' : 'border-stone-700 text-stone-400'}`}>
                {t || 'ყველა'}
              </button>
            ))}
          </div>
          {loading ? (
            ...
            <div className="text-center py-16 text-stone-500"><Receipt size={40} className="mx-auto mb-3 opacity-30" /><p>ტრანზ. არ მოიძ.</p></div>
          ) : (
            <div className="space-y-2">
              {transactions.map((t, i) => (
                <div key={i} className="flex items-center gap-4 bg-stone-900/80 border border-stone-800/50 rounded-xl px-5 py-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-stone-300 text-sm">{t.reference_type || '—'}</p>
                    {t.counterparty_tin && <p className="text-stone-500 text-xs">სს/კ: {t.counterparty_tin}</p>}
                  </div>
                  <div className="text-right shrink-0">
                    <p className="text-stone-400 text-xs">{GEL(t.taxable_amount)} × {t.vat_rate}%</p>
                    <p className="font-semibold text-white">{GEL(t.vat_amount)}</p>
                  </div>
                  <p className="text-stone-500 text-xs w-24 text-right shrink-0">{new Date(t.transaction_date).toLocaleDateString('ka-GE')}</p>
                </div>
              ))}
            </div>
          )}
