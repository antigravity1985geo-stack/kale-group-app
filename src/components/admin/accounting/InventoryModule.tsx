...
const TYPE_STYLES: Record<string, string> = {
  ...
  OPENING:        'text-stone-300 bg-stone-800/50 border-stone-700/40',
};
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-white flex items-center gap-2"><Warehouse size={22} /> სასაქონლო მარაგი</h2>
          <p className="text-stone-400 text-sm mt-1">Stock Levels · FIFO · Adjustments</p>
        </div>
        <div className="text-right">
          <p className="text-stone-500 text-xs">სულ ღირ.</p>
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="bg-stone-900 border border-stone-800 rounded-xl p-4">
          <p className="text-stone-400 text-xs mb-1">სულ პოზიცია</p>
          <p className="text-white font-bold text-xl">{levels.length}</p>
        </div>
        <div className={`rounded-xl p-4 border ${lowStock.length > 0 ? 'bg-red-900/20 border-red-800/40' : 'bg-stone-900 border-stone-800'}`}>
          <p className="text-stone-400 text-xs mb-1 flex items-center gap-1">{lowStock.length > 0 && <AlertTriangle size={12} className="text-red-400" />}Low Stock</p>
          <p className={`font-bold text-xl ${lowStock.length > 0 ? 'text-red-300' : 'text-emerald-300'}`}>{lowStock.length}</p>
        </div>
        <div className="bg-stone-900 border border-stone-800 rounded-xl p-4">
          <p className="text-stone-400 text-xs mb-1">მარაგის ღირ.</p>
          ...
          {loading ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead><tr className="text-stone-500 text-xs border-b border-stone-800">
                ...
                <tbody>
                  {filteredLevels.map(l => {
                    ...
                    return (
                      <tr key={l.id} className={`border-b border-stone-800/50 hover:bg-stone-800/20 transition-colors ${isLow ? 'bg-red-900/5' : ''}`}>
                        <td className="py-3 px-4 text-white">{l.products?.name || l.product_id?.slice(0, 8) + '...'}</td>
                        <td className="py-3 px-4 text-right text-stone-300">{l.quantity_on_hand}</td>
                        ...
                        <td className="py-3 px-4 text-right text-stone-400">{l.reorder_point}</td>
                        <td className="py-3 px-4 text-right text-stone-300">{GEL(l.avg_cost || 0)}</td>
        ...
        loading ? (
          ...
          <div className="space-y-2">
            {transactions.map(t => {
              ...
              return (
                <div key={t.id} className="flex items-center gap-4 bg-stone-900/80 border border-stone-800/50 rounded-xl px-5 py-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-white text-sm">{t.products?.name || 'პროდ. ' + t.product_id?.slice(0, 8)}</p>
                    <p className="text-stone-500 text-xs">{t.notes || '—'}</p>
                  </div>
                  ...
                  <div className="text-right">
                    ...
                    {t.total_cost && <p className="text-stone-500 text-xs">{GEL(t.total_cost)}</p>}
                  </div>
                  <p className="text-stone-500 text-xs w-24 text-right">{new Date(t.created_at).toLocaleDateString('ka-GE')}</p>
                </div>
              );
            })}
          </div>
      ...
      {/* Adjustment Tab */}
      {tab === 'adjust' && (
        <form onSubmit={handleAdjust} className="bg-stone-900/80 border border-stone-800 rounded-2xl p-6 space-y-4 max-w-lg">
          <h3 className="text-white font-semibold">📦 მარაგის კორექტირება</h3>
          <div>
            <label className="text-stone-400 text-xs mb-1 block">პროდუქტი *</label>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="text-stone-400 text-xs mb-1 block">ტიპი *</label>
            ...
            <div>
              <label className="text-stone-400 text-xs mb-1 block">რაოდ. *</label>
              <input type="number" min="0.01" step="0.01" value={adjForm.quantity} onChange={e => setAdjForm({ ...adjForm, quantity: e.target.value })} required
                className="w-full bg-stone-800 border border-stone-700 rounded-xl px-3 py-2 text-white text-sm focus:outline-none focus:border-amber-600" placeholder="0" />
            </div>
          </div>
          <div>
            <label className="text-stone-400 text-xs mb-1 block">ერთ. ღირ. (ლარი)</label>
            <input type="number" min="0" step="0.01" value={adjForm.unit_cost} onChange={e => setAdjForm({ ...adjForm, unit_cost: e.target.value })}
              className="w-full bg-stone-800 border border-stone-700 rounded-xl px-3 py-2 text-white text-sm focus:outline-none focus:border-amber-600" placeholder="0.00" />
          </div>
          <div>
            <label className="text-stone-400 text-xs mb-1 block">შენიშვნა</label>
            <input value={adjForm.notes} onChange={e => setAdjForm({ ...adjForm, notes: e.target.value })}
              className="w-full bg-stone-800 border border-stone-700 rounded-xl px-3 py-2 text-white text-sm focus:outline-none focus:border-amber-600" placeholder="კორ. მიზეზი..." />
          </div>
          <button type="submit" className="w-full py-2.5 bg-amber-600 hover:bg-amber-500 text-white rounded-xl text-sm font-medium transition-all">
