...
const TYPE_STYLES: Record<string, string> = {
  ...
  OPENING:        'text-slate-600 bg-admin-bg/50 border-admin-muted/10/40',
};
  ...
  return (
    <div className="admin-fade-in space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-admin-text flex items-center gap-2"><Warehouse size={22} /> სასაქონლო მარაგი</h2>
          <p className="text-admin-muted text-sm mt-1">Stock Levels · FIFO · Adjustments</p>
        </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="bg-white border-none shadow-sm rounded-2xl focus:ring-4 focus:ring-admin-primary/5 transition-all p-4">
          <p className="text-admin-muted text-xs mb-1">სულ პოზიცია</p>
          <p className="text-admin-text font-bold text-xl">{levels.length}</p>
        </div>
        <div className={`rounded-xl p-4 border ${lowStock.length > 0 ? 'bg-red-900/20 border-red-800/40' : 'bg-white shadow-sm border-admin-muted/10'}`}>
          <p className="text-admin-muted text-xs mb-1 flex items-center gap-1">{lowStock.length > 0 && <AlertTriangle size={12} className="text-red-400" />}Low Stock</p>
          <p className={`font-bold text-xl ${lowStock.length > 0 ? 'text-red-300' : 'text-emerald-300'}`}>{lowStock.length}</p>
        </div>
        <div className="bg-white border-none shadow-sm rounded-2xl focus:ring-4 focus:ring-admin-primary/5 transition-all p-4">
          <p className="text-admin-muted text-xs mb-1">მარაგის ღირ.</p>
          ...
          {loading ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead><tr className="text-slate-400 text-xs border-b border-admin-muted/10">
                ...
                <tbody>
                  {filteredLevels.map(l => {
                    ...
                    return (
                      <tr key={l.id} className={`border-b border-admin-muted/10/50 hover:bg-slate-100/50/20 transition-colors ${isLow ? 'bg-red-900/5' : ''}`}>
                        <td className="py-3 px-4 text-admin-text">{l.products?.name || l.product_id?.slice(0, 8) + '...'}</td>
                        ...
                        <td className="py-3 px-4 text-right text-admin-muted">{l.reorder_point}</td>
        ...
        loading ? (
          ...
          <div className="space-y-2">
            {transactions.map(t => {
              ...
              return (
                <div key={t.id} className="flex items-center gap-4 bg-white shadow-[0_8px_30px_rgba(0,0,0,0.02)] rounded-2xl border border-admin-muted/5 hover:shadow-lg transition-all px-5 py-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-admin-text text-sm">{t.products?.name || 'პროდ. ' + t.product_id?.slice(0, 8)}</p>
      ...
      {/* Adjustment Tab */}
      {tab === 'adjust' && (
        <form onSubmit={handleAdjust} className="bg-white shadow-sm/80 border border-admin-muted/10 rounded-2xl p-6 space-y-4 max-w-lg">
          <h3 className="text-admin-text font-semibold">📦 მარაგის კორექტირება</h3>
          <div>
            <label className="text-brand-600 font-bold tracking-widest text-[10px] uppercase mb-2 block">პროდუქტი *</label>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="text-brand-600 font-bold tracking-widest text-[10px] uppercase mb-2 block">ტიპი *</label>
            ...
            <div>
              <label className="text-brand-600 font-bold tracking-widest text-[10px] uppercase mb-2 block">რაოდ. *</label>
              <input type="number" min="0.01" step="0.01" value={adjForm.quantity} onChange={e => setAdjForm({ ...adjForm, quantity: e.target.value })} required
                className="w-full bg-admin-bg border border-admin-muted/10 rounded-xl px-3 py-2 text-admin-text text-sm focus:outline-none focus:border-gold-500 focus:bg-white transition-all shadow-sm" placeholder="0" />
            </div>
          </div>
          <div>
            <label className="text-brand-600 font-bold tracking-widest text-[10px] uppercase mb-2 block">ერთ. ღირ. (ლარი)</label>
            <input type="number" min="0" step="0.01" value={adjForm.unit_cost} onChange={e => setAdjForm({ ...adjForm, unit_cost: e.target.value })}
              className="w-full bg-admin-bg border border-admin-muted/10 rounded-xl px-3 py-2 text-admin-text text-sm focus:outline-none focus:border-gold-500 focus:bg-white transition-all shadow-sm" placeholder="0.00" />
          </div>
          <div>
            <label className="text-brand-600 font-bold tracking-widest text-[10px] uppercase mb-2 block">შენიშვნა</label>
            <input value={adjForm.notes} onChange={e => setAdjForm({ ...adjForm, notes: e.target.value })}
              className="w-full bg-admin-bg border border-admin-muted/10 rounded-xl px-3 py-2 text-admin-text text-sm focus:outline-none focus:border-gold-500 focus:bg-white transition-all shadow-sm" placeholder="კორ. მიზეზი..." />
          </div>
          <button type="submit" className="w-full py-2.5 bg-brand-600 hover:bg-admin-bg0 text-admin-text rounded-xl text-sm font-medium transition-all">
