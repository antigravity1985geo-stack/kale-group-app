...
if (asideStart !== -1 && asideEnd !== -1) {
  const newSidebar = \`      <aside className="w-[300px] bg-admin-sidebar text-white flex flex-col shadow-2xl z-20 relative border-r border-white/5">
  ...
  const before = content.substring(0, asideStart);
  const after = content.substring(asideEnd);
  
  fs.writeFileSync('src/AdminPanel.tsx', before + newSidebar + after, 'utf8');
  console.log('Sidebar Premium Patch Applied Successfuly');
} else {
