...
function OffcutCard({ offcut, onDispose, onSelect }) {
  ...
  const daysOld = Math.floor((Date.now() - new Date(offcut.created_at)) / 86400000)
...
export default function OffcutInventory({ onSelectOffcut }) {
  ...
  const sorted = useMemo(() => {
    ...
    switch (sortBy) {
      ...
      case 'age_asc':   return arr.sort((a, b) => new Date(a.created_at) - new Date(b.created_at))
      case 'age_desc':  return arr.sort((a, b) => new Date(b.created_at) - new Date(a.created_at))
      default:          return arr
    }
  }, [offcuts, sortBy])
  ...
  const stats = useMemo(() => ({
    ...
    agingCount:     offcuts.filter(o => {
                      const d = Math.floor((Date.now() - new Date(o.created_at)) / 86400000)
                      return d > 270
                    }).length,
  }), [offcuts])
  ...
  const selectStyles = {
    width: '100%', background: '#0d0d16', border: `1px solid ${G.glassBorder}`,
